...
def _otbn_compile_jazz_sources(ctx, additional_srcs = []):
    """Helper function that, for each jasmin source file in the provided
    context, adds an action to the context that invokes jasminc, producing a
    corresponding assembly file. Returns a list of all assembly  files that will
    be generated by these actions.
    """

    asms = []
    for src in ctx.files.srcs + additional_srcs:
        asm = ctx.actions.declare_file(src.basename.replace("." + src.extension, ".s"))
        asms.append(asm)
        ctx.actions.run(
            outputs = [asm],
            inputs = ([src] +
                      [ctx.executable._otbn_jasminc] + 
                      [f for dep in ctx.attr.deps for f in dep.files.to_list()]),
...
def _otbn_jasminc_asm(ctx):
    asms = _otbn_compile_jazz_sources(ctx)

    return [
        DefaultInfo(
            files = depset(asms),
            data_runfiles = ctx.runfiles(files = asms),
        ),
    ]
...
otbn_jasminc_asm = rv_rule(
    implementation = _otbn_jasminc_asm,
    attrs = {
        "srcs": attr.label_list(allow_files = True),
        "deps": attr.label_list(allow_files = True),
        "args": attr.string_list(),
        "_otbn_jasminc": attr.label(
            default = "//hw/ip/otbn/util:otbn_jasminc",
            executable = True,
            cfg = "exec",
        ),
    }
